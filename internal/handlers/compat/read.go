...
func (h *ReadHandler) ListWarehouses(c *gin.Context) {
	...
	rows, err := pool.Query(ctx, `
		SELECT code, COALESCE(name_1,'') FROM ic_warehouse WHERE COALESCE(status,0)=0 ORDER BY code`)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": err.Error()})
		return
	}
	...
	shelfRows, err := pool.Query(ctx, `
		SELECT whcode, code, COALESCE(name_1,'')
		FROM ic_shelf
		WHERE COALESCE(status,0)=0
		ORDER BY whcode, code`)
	if err != nil {
		api.Internal(c, "warehouse_shelves_query_failed", "query warehouse shelves failed", err.Error())
		return
	}
	...
	for shelfRows.Next() {
		var whCode string
		var s shelfItem
		if err := shelfRows.Scan(&whCode, &s.Code, &s.Name); err != nil {
			api.Internal(c, "warehouse_shelf_scan_failed", "scan warehouse shelf failed", err.Error())
			return
		}
		if idx, ok := byCode[whCode]; ok {
			warehouses[idx].Shelves = append(warehouses[idx].Shelves, s)
		}
	}
	api.OKPage(c, warehouses, len(warehouses), 1, len(warehouses))
}
...
func (h *ReadHandler) GetWarehouse(c *gin.Context) {
	...
	err := pool.QueryRow(ctx, `
		...
		Scan(&w.Code, &w.Name)
	if err != nil {
		api.NotFound(c, "warehouse_not_found", "warehouse not found")
		return
	}

	rows, err := pool.Query(ctx, `
		SELECT code, COALESCE(name_1,'') FROM ic_shelf WHERE whcode=$1 AND COALESCE(status,0)=0 ORDER BY code`, code)
	if err == nil {
		defer rows.Close()
		for rows.Next() {
			var s shelfItem
